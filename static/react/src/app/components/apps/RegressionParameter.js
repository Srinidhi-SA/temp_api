...
export class RegressionParameter extends React.Component {
    ...
    changeFoldCheck(min,max,name,e){
        let arrName = ["regParam","elasticNetParam","threshold","minInfoGain","smoothing"]
        if(e.target.value < min || e.target.value > max){
            e.target.parentElement.lastElementChild.innerHTML = "Valid Range is "+min+"-"+ max
        }else if(!Number.isInteger(parseFloat(e.target.value)) && !arrName.includes(name) ){
    ...
    changeSliderValue(e) {
        ...
        document.getElementsByClassName(this.state.name)[0].parentElement.querySelector(".range-validate").innerText = ""
        this.props.dispatch(updateAlgorithmData(this.props.algorithmSlug,this.props.parameterData.name,e.target.value,this.props.type));
    }
    ...
    changeTextboxValue(e){
        ...
        //Causing issue while running validate input function
        ...
        this.props.dispatch(updateAlgorithmData(this.props.algorithmSlug,this.props.parameterData.name,e.target.value,this.props.type));
        var numbers = /^[0-9\s]*$/;
        if( ($(".hiddenLayerCls").val() != undefined) && ($(".hiddenLayerCls").val()) < 0 ){
            document.getElementById("error").innerHTML="negative value not allowed";
        } else if( ($(".hiddenLayerCls").val() != undefined) && !numbers.test($(".hiddenLayerCls").val())){
            document.getElementById("error").innerHTML="only number allowed";
        } else if( ($(".hiddenLayerCls").val() != undefined) && $(".hiddenLayerCls").val() == ""){
    ...
    renderParameterData(parameterData,tune){ 
                        ...
                        return (
                            <div className="row">
                                <div className="col-md-2">
                                    <input type={type} className={classN} onKeyDown={ (evt) => evt.key === 'e' && evt.preventDefault() } value={this.state.defaultVal} onChange={this.changeTextboxValue.bind(this)} onBlur={this.checkChangeTextboxValue.bind(this,this.state.min,this.state.max,parameterData.expectedDataType)} onInput={this.validateInput.bind(this,this.state.min,this.state.max,this.state.name)}/>
                                    <div className="clearfix"></div>
                        ...
                        if(tune){
                            ...
                            return (
                                <div className="row">                        
                                    ...
                                    <div className="col-md-4 col-sm-4"><input type="number" onKeyDown={ (evt) => evt.key === 'e' && evt.preventDefault() } min = {this.state.min} max = {this.state.max} className={sliderTextCls} value={this.state.defaultVal} onChange={this.changeSliderValueFromText.bind(this)} onInput={this.changeFoldCheck.bind(this,this.state.min,this.state.max,this.state.name)} onBlur={this.checkChangeTextboxValue.bind(this,this.state.min,this.state.max,dataTypes)}/>
                                        <div className="clearfix"></div>
                                        <div className="range-validate text-danger"></div>
                                    </div>
                                </div>
                    ...
                    case "boolean":
                        ...
                        return ( 
                            <div className="ma-checkbox inline">
                                <input  type="checkbox" id={chkBox} name={chkBox} onChange={this.handleCheckboxEvents.bind(this)} checked={this.state.defaultVal}/><label htmlFor={chkBox}>&nbsp;</label>
                            </div>
                        );
                        break;
                    default:
        ...
        if(this.props.algorithmData.filter(i=>i.algorithmName==="Neural Network (Sklearn)").length!=0 &&  this.props.algorithmData.filter(i=>i.algorithmName==="Neural Network (Sklearn)")[0].hyperParameterSetting[0].selected == false){
            if(e.target.classList[1]=="fractionCls" && !numbers.test($('.fractionCls').val())){
                return {"iserror":true,"errmsg":"only numbers allowed"};
            }
        }
        
        if(this.props.algorithmData.filter(i=>i.algorithmName==="Neural Network (Sklearn)").length!=0 && this.props.algorithmData.filter(i=>i.algorithmName==="Neural Network (Sklearn)")[0].hyperParameterSetting[0].selected == true){
        ...
        if(this.props.algorithmData.filter(i=>i.algorithmName==="Neural Network (Sklearn)").length!=0 && this.props.algorithmData.filter(i=>i.algorithmName==="Neural Network (Sklearn)")[0].hyperParameterSetting[1].selected){
