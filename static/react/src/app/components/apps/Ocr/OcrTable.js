        ...
        return (
          <tr id={index}>
            ...
            <td style={item.status == "Ready to recognize" ? { cursor: 'not-allowed' } : { cursor: 'pointer' }}>
              <Link style={item.status == "Ready to recognize" ? { pointerEvents: 'none' } : { pointerEvents: 'auto' }} to={item.name} onClick={() => { this.handleImagePageFlag(item.slug,item.name) }} title={item.name}>{item.name}</Link>
            ...
            <div class="col-sm-6 text-right">
              <div class="form-inline">
				...
				<ReactTooltip place="left" type="light" id="exportTip"/>
                  <ul className="export btn btn-default" >
                  <li class="dropdown">
