    ...
    #    img_array = np.array(Image.fromarray(image).convert('1')).astype('uint8')
        ...
        if len(index) >=1:
            ...
            if sum([1 if crop_index + i in index else 0 for i in range(-5,6)]) >=10:
                ...
                optness = {ind : sum([1 if ind + i in index else 0 for i in range(-5,6)]) for ind in range(crop_index-10,crop_index+11) if ind in index}  # Checking in selected pixel +10 pixels for better index
