    ...
    if type == 2:
            ...
            for line_number in p1_p3:
                ...
                if (line_number not in clubbed_lines):
                    ...
                    if ([re.match(r'.*([2][0][0-9]{2})', line) for line in x] + [re.match(r'.*([1][9][0-9]{2})', line)
                            ...
                            x)) and (
                            [re.match(r'.*(Fall|Spring|Summer|Term)', word) for word in x] != [None] * len(x)) and (
                            [re.match(r'.*(/)', word) for word in x] == [None] * len(x)):
        ...
        for line_number in p1_p3:
            ...
            if (line_number not in clubbed_lines):
                ...
                if ([re.match(r'.*([2][0][0-9]{2})', line) for line in x] + [re.match(r'.*([1][9][0-9]{2})', line) for
                                                                             line in x] != [None] * 2 * len(x)) and (
                        [re.match(r'.*(Fall|Spring|Summer|Term)', word) for word in x] != [None] * len(x)) and (
                        [re.match(r'.*(/)', word) for word in x] == [None] * len(x)):
                    ...
                    try:
                        ...
                        elif re.findall(r'Total|Cum|Term GPA', joined) and (
                                [w[0].isdigit() for w in x] != [False] * len(x)):
                            sem_info[key]['final_' + str(i1)] = [x, sl]
                            i1 = i1 + 1
...
def clean_info(x, lines_api, wc, p1_p3):
    ...
    for sem in x:
        ...
        for data in data_rows:
            ...
            if ((s[data][0][1][0].isdigit()) or (s[data][0][1][0] == 'D')) and (len(s[data][0][1]) > 1) and (
            (len(s[data][0][0].split()) <= 1)) and (s[data][0][0][-1] != ':'):
            ...
            if len(s[data][0]) == len(s['sem_header_1'][0]):
                ...
                for i, heading in enumerate(s['sem_header_1'][0]):
                    ...
                    for j, cord in enumerate(x1_x2):
                        ...
                        if found == False:
                            if (cord[0] in range(x1 - 15, x1 + 16)) or (cord[1] in range(x2 - 15, x2 + 16)) or (
                                    abs(sum(cord) - sum((x1, x2))) <= 25):
...
def clean(x):
    ...
    if len(words) == 2:
        ...
        ## BOTH ARE NUMBERS AND '.' (present)
        elif ((words[0].isdigit() and words[1][1].isdigit()) or (words[0][0].isdigit() and words[1].isdigit())) and (
